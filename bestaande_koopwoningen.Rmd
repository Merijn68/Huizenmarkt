...
Factoren die het aantal transactie bepalen
...
df <- 
  ...
  full_join(voorraad_woningen, by = c("date" = "Perioden_Date")) %>%
  select (date, aantal_huishoudens, EindstandVoorraad_8) %>%
  set_names('date','aantal_huishoudens','eindstand_voorraad') %>%
  mutate(te_kort = (eindstand_voorraad - aantal_huishoudens)) %>%
...
bp <- 
  ...
  filter (variable == 'te_kort') %>%
  ggplot(aes (x = date)) +
  geom_bar(stat = "identity" , aes(y = value, color = variable, fill = variable )) +
...
Daarnaast zien we landelijk ook nog een fors aantal leegstaande woningen. 
...
select(Perioden_Date, StatusVanBewoning, TotaleWoningvoorraad_1) %>%
filter (StatusVanBewoning =='A028726') %>%
mutate(totaal = TotaleWoningvoorraad_1 / 1000) %>%
ggplot(aes (x = Perioden_Date)) +
geom_line(aes(y=totaal, color = StatusVanBewoning)) +
labs(
...
```{r}
voorraad_woningen_naar_eigendom %>%
  filter (StatusVanBewoning =='T001235') %>%
  filter( Perioden_Date == as.Date("2020-01-01")) %>%
  select(Perioden_Date, 
       Koopwoningen_2,
       EigendomWoningcorporatie_4, 
...
Steeds meer alleenstaanden > 65 + en migratie
...
-- prognose nieuwbouwhuizen toevoegen ---
...
voorraad_woningen %>%
  select(Perioden_Date,
         Nieuwbouw_2) %>%
  mutate(nieuwbouw = Nieuwbouw_2 / 1000) %>%
  filter(Perioden_Date > as.Date("1980-01-01")) %>%
  ggplot(aes(x=Perioden_Date, y = nieuwbouw )) +
...
In lijn met ontwikkeling aantal huishoudens?
